...
public class lab9b {
	...
	public static int addItems(Scanner sIn, ArrayList<String> shoppingList) {
		...
		item = answer.substring(0,semicolon).trim();
		numbers = answer.substring(semicolon+1).trim();
		System.out.print(item+ ":"+ numbers +" has been added to the Shopping List"+"\n");
		shoppingList.add(item +":"+ numbers);
	...
	public static void showItems(ArrayList<String> shoppingList) {
		...
		for (int i = 0; i< shoppingList.size(); i++) {
			subList = shoppingList.get(i);
			int semicolon = subList.indexOf(":");
			Item = subList.substring(0,semicolon);
			Quantity = subList.substring(semicolon+1);
			System.out.printf("%4s %-5s %n",Item,Quantity);
		}
